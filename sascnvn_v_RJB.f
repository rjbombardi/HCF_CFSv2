...
!
     ...
     &,             eps => con_eps, epsm1 => con_epsm1
      !---- Modified by Rodrigo Bombardi
      USE BCLTRIGGER, ONLY : TRIGGERFLG, bcl_mixed_layer 
...
!
      !****************** Added by Rodrigo J. Bombardi******************
      real(kind=kind_phys), allocatable :: prss(:,:)
      real(kind=kind_phys), allocatable :: t_def(:), bcl_p(:)
c-----------------------------------------------------------------------
!
      km1 = km - 1
c
...
!rjb start change
!
      IF(TRIGGERFLG)THEN  ! Modified R. Bombardi
      !****************************************************
      !*** Tawfic and Diermayer triggering mechanism    ***
      !***                                              ***
      !*** Implemented by R. Bombardi on Jan 2014       ***
      !***                                              ***
      !*** Purpose:                                     ***
      !*** Calls a new test for convection based on the ***
      !*** buoyant condensation level (BCL)             ***
      !*** This method assumes moist deep convection is ***
      !*** triggered when saturation occurs at the top  ***
      !*** of the mixed layer.  Mixed layer is defined  ***
      !*** by the neutral buoyancy level of a surface   ***
      !*** parcel (usually 2-meter parcels).            ***
      !***                                              ***
      !*** Reference: Tawfik and Dirmeyer 2013 GRL: A   ***
      !*** processed based framework for quantifying    ***
      !*** the atmospheric background state of surface  ***
      !*** triggered convection                         ***
      !****************************************************
 
         allocate(prss(IM,KM),t_def(IM),bcl_p(IM))
!        temp(:,1)=TMP2M(:)
!        temp(:,:) = TO(:,:)

!        shum(:,1)=QV2M(:)
!        shum(:,:)=QO(:,:)
      ! convert surface pressure to Pa from hPa
!        prss(:,1)=PS(:)*1000.
        prss(:,:) = pfld(:,:) * 100.0
      !****************************************************
      !*** call generic subroutine for finding          ***
      !*** neutral buoyant level given profiles         ***
      !*** and saturation deficit at neutral level      ***
      !*** To find BCL, find first level when sat_def<=0***
      !*** plugging in current 2m pot temp then an      ***
      !*** estimate of the PBL height is returned       ***
      !****************************************************

      ! Do a temp increment to find where t_def and to converge on
      ! the BCL height (defined as level where humidity deficit~=0)
       do I = 1, IM
         call bcl_mixed_layer(KM,TO(I,:),PRSS(I,:)
     &,                  QO(I,:),t_def(I),BCL_P(I))
       enddo
      !*************************************************************
      !***           LOOK FOR CONVECTIVE CLOUD BASE              ***
      !***          AS THE BUOYANCY CONVECTIVE LEVEL             ***
      !*************************************************************
!        DO I=1,IM
!          FLG(I)    = CNVFLG(I)
!          KBCON(I)  = KMAX(I)
...
!        DO K = 1, KM1
!          DO I=1,IM
!            if (k .le. kbmax(i) .and. t_def(i) .lt. 0.0) then
!              IF(FLG(I).AND.K.GT.KB(I).AND.PFLD(I,k).LE.BCL_P(I)) THEN
!                  FLG(I)   = .FALSE.
!                  KBCON(I) = K
!                ENDIF
!              ENDIF
...
!      ELSE
       ...
       ENDIF
      !*************************************************************
      !***           CONVECTIVE TRIGGERING MECHANISM             ***
      !***                                                       ***
      !***           LOOK FOR CONVECTIVE CLOUD BASE              ***
      !***          AS THE LEVEL OF FREE CONVECTION              ***
      !*************************************************************
!
...
c
      ...
      do i=1,im
        if(cnvflg(i)) then
          ...
          IF(.NOT. TRIGGERFLG)THEN
            if(pbcdif(i).gt.cincr) cnvflg(i) = .false.
          ELSE
            if(t_def(i).ge.0.0) cnvflg(i) = .false.
          ENDIF
        endif
      enddo
!!
      IF(TRIGGERFLG) deallocate(prss,t_def,bcl_p)   !rjb
...
!          if(k.gt.kbcon1(i).and.dbyo(i,k-1).gt.0. .and.
!     &       dbyo(i,k).le.0. .and. dbyo(i,k+1).lt.0.) then
!---- rjb
             ktcon(i) = k
             flg(i)   = .false.
          endif
